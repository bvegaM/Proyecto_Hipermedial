<?php
	...
	if($eventoId == 1){
		...
		$sql1="INSERT INTO T_FACTURA_DETALLE(0,'boleto General',$cantidad,$precio,$row['fc_id'],$evento)";
	...
	if($eventoId == 2){
		...
		$resultadoF=$conn->query($factura);
		$row=$resultadoF->fetch_assoc();
		$sql1="INSERT INTO T_FACTURA_DETALLE(0,'boleto TRIBUNA',$cantidad,$precio,$row['fc_id'],$evento)";
		$result1 =$conn->query($sql1);
	}
	if($eventoId == 3){
		$factura="SELECT *
		...
		$resultadoF=$conn->query($factura);
		$row=$resultadoF->fetch_assoc();
		$sql1="INSERT INTO T_FACTURA_DETALLE(0,'boleto VIP',$cantidad,$precio,$row['fc_id'],$evento)";
		$result1 =$conn->query($sql1);
	}
	if($eventoId == 4){
		$factura="SELECT *
		...
		$resultadoF=$conn->query($factura);
		$row=$resultadoF->fetch_assoc();
		$sql1="INSERT INTO T_FACTURA_DETALLE(0,'boleto BOX',$cantidad,$precio,$row['fc_id'],$evento)";
		$result1 =$conn->query($sql1);
	}
