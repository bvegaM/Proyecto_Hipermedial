<?php
	...
	if($eventoId == 2){
		$id = $conn->insert_id;
		$facturaD = "INSERT INTO T_FACTURA_DETALLE VALUES(0,'BOLETO PARA TRIBUNA',$cantidad,$precio,$id,$evento)";
		$resultFD = $conn->query($facturaD);
		header("Location: ../../vista/mis_compras.php?codigo=".$codigo);
	}
	if($eventoId == 3){
		$id = $conn->insert_id;
		$facturaD = "INSERT INTO T_FACTURA_DETALLE VALUES(0,'BOLETO PARA VIP',$cantidad,$precio,$id,$evento)";
		$resultFD = $conn->query($facturaD);
		header("Location: ../../vista/mis_compras.php?codigo=".$codigo);
	}
	if($eventoId == 4){
		$id = $conn->insert_id;
		$facturaD = "INSERT INTO T_FACTURA_DETALLE VALUES(0,'BOLETO PARA BOX',$cantidad,$precio,$id,$evento)";
		$resultFD = $conn->query($facturaD);
		header("Location: ../../vista/mis_compras.php?codigo=".$codigo);
	}
